...
不满意反馈重生成，可多轮）→ ffmpeg 渲染（镜头锁定即后台预渲染，备选随主选
一同导出，manifest 标 role；最终「确认渲染」通常只剩补漏 + 打包）→ 下载 zip。
...
- **shot_render（预渲染）失败**：它是纯优化，不影响项目状态 —— 最终渲染会
  重剪同一片段。只有反复失败才值得排查（`job_run.error`），
  常见原因与渲染失败相同（原片被移动、磁盘满）。
