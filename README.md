...
⑤ 视频检索   1fps 抽帧 → 同一 embedding 服务 → tracklet 时间段 → 命中时给出
             「03:12–03:45 出现」并可直达源站视频对应时间点
```
...
- **视频按 1 fps 抽帧检索**（见 `docs/plans/0008`）：能给出「此人在 03:12–03:45 出现」
  的时间段，但**短于 1 秒的一闪而过会漏**；运动模糊与压缩也让视频召回率低于照片。
  不够用时调 `VIDEO_SAMPLE_FPS`（成本线性）。超长/超大视频会被跳过
  （`VIDEO_MAX_DURATION_S` / `VIDEO_MAX_BYTES`）。
